(function ($, app) {
	...
	var TablesModel = (function () {
		...
		TablesModel.prototype.setHistorySettings = function (id, settings) {
			if(STB_DATA.isPro && settings.length) {
				if (isNaN(id = parseInt(id))) {
					throw new Error('Invalid table id.');
				}
		...
		TablesModel.prototype.getPreview = function (id, preview) {
			...
			if(container.length) {
				...
				return this.render(app.getParameterByName('id')).done(function(response) {
					...
					if(table.find('td.editable') && typeof(STB_DATA.pluginsUrl) != 'undefined') {
						$.getScript(STB_DATA.pluginsUrl + '/tables-generator-pro/src/SupsysticTablesPro/Tables/assets/js/tables.shortcode.pro.js');
					}
				});
			}
		};
