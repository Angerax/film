...
class SupsysticTables_Core_Module extends SupsysticTables_Core_BaseModule
{
	...
	public function addCommomPluginData() {
		$environment = $this->getEnvironment();
		$config = $environment->getConfig();
		$pro = $environment->isPro();
		...
		wp_localize_script('tables-core', 'STB_DATA', $jsData);
	}
    ...
    public function afterUiLoaded(SupsysticTables_Ui_Module $ui)
    {
		...
		add_action(is_admin() ? $hookName : 'wp_enqueue_scripts', array($this, 'addCommomPluginData'), 999);
        $ui->add(
